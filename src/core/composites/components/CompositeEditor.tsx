...
// keeps size small - this will import all of the icons and bundle them
...
export const CompositeEditor: React.FC<Props> = ({ item, context }) => {
  ...
  const removeComposite = (compositeIndex: number) => {
    ...
    const newstuff = [...tracker];
    let removeIndex = 0;
    for (let i = 0; i < newstuff.length; i++) {
      if (newstuff[i].order === compositeIndex) {
        removeIndex = i;
        break;
      }
    }
    newstuff.splice(removeIndex, 1);
    // reorder
    for (let i = 0; i < newstuff.length; i++) {
      console.log(`old index ${newstuff[i].order}`);
      newstuff[i].order = i;
      console.log(`new index ${newstuff[i].order}`);
    }
    setTracker([...newstuff]);
  ...
  const toggleOpener = (index: number) => {
    const newstuff = [...isOpen];
    newstuff[index] = !newstuff[index];
    setIsOpen([...newstuff]);
  };
