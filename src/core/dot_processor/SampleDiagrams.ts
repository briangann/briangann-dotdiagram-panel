...
class SampleDiagrams {
  ...
  Diagram3 = `digraph {
    rankdir = LR;
    node [fontcolor=white,color=green,shape=doublecircle]; ANode CNode;
    node [fontcolor=orange,color=yellow,shape = circle]; D;
    edge [arrowhead=vee,color=white,fontcolor=white];
    ANode -> CNode [ arrowhead=dot, color=red, fontcolor=red,label="AtoCEdge" ];
    ANode -> B [ label = "AtoB" ];
    B -> CNode [ label = "BtoC" ];
    CNode -> D [ label = "CtoD" ];
    }`;
